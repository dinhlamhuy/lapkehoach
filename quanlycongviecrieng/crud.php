...
$i = 1;
$manvht = (isset($_POST['manvht'])) ? $_POST['manvht'] : '';
...
switch ($option) {
    case 1:
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            if ($result) {
                ...
                $data[] = ['alert'  =>   'success:Thêm công việc mới thành công'];
            } else {
            ...
            nvlkh.nv_ma as nvlkh_ma, nvlkh.nv_hoten as nvlkh_hoten, nvlkh.nv_email as nvlkh_email, 
                ...
                FROM `kehoachcongviec` khcv INNER JOIN `nhanvien` nvth ON nvth.nv_ma=khcv.nv_thuchien INNER JOIN nhanvien nvlkh ON nvlkh.nv_ma=khcv.nv_lapkehoach INNER JOIN `duan` ON khcv.da_ma=duan.da_ma WHERE nvth.nv_ma='$manvht' ORDER BY ma_cv DESC LIMIT 1
    EOT;
            ...
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                ...
                $data[] = [
                    ...
                    'khcv_noidung' =>  '<div class="khcv_noidung" data-id="' . $row['noidung_cv'] . '" >' . $row['noidung_cv'] . '</div>',
                    'khcv_ghichu' =>  '<div class="khcv_ghichu" data-id="' . $row['ghichu_cv'] . '" >' . $row['ghichu_cv'] . '</div>',
    ...
    case 2:
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            if (timidtuan($row_nvth['khcv_thoihanhoanthanh']) < timidtuan($ngayketthuc)) {
                ...
                add_new_history('Gia hạn', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã gia hạn thêm cho công việc có mã CV' . sprintf('%03d', $khcv_ma) . ' từ ' . $row_nvth['khcv_thoihanhoanthanh'] . ' Đến ' . $ngayketthuc . '', $nvthuchien, $khcv_ma);
            } else  if (timidtuan($row_nvth['khcv_thoihanhoanthanh']) > timidtuan($ngayketthuc)) {
                rutnganthoihancongviec($khcv_ma,  $ngayketthuc);

                add_new_history('Rút ngắn thời gian hoàn thành', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã rút ngắn thời gian hoàn thành của công việc có mã CV' . sprintf('%03d', $khcv_ma) . ' từ ' . $row_nvth['khcv_thoihanhoanthanh'] . ' Đến ' . $ngayketthuc . '', $nvthuchien, $khcv_ma);
            }
            //thay đổi nhân viên
            if ($row_nvth['nv_thuchien'] != $nvthuchien) {
                add_new_history('Chuyển giao nhiệm vụ', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã bàn giao lại công việc có mã CV' . sprintf('%03d', $khcv_ma) . ' do NV' . sprintf('%03d', $row_nvth['nv_thuchien']) . ' thực hiện trước đó và chuyển đến cho NV' . sprintf('%03d', $nvthuchien) . ' thực hiện tiếp', $nvthuchien, $khcv_ma);
            }
            // Thiết lập trạng thái
            if (strcasecmp($row_nvth['khcv_tiendo'], $trangthaicv) != 0) {
                if (strcasecmp($trangthaicv,'Hoàn thành')==0) {
                    
                    add_new_history('Hoàn thành công việc', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã hoàn thành công việc có mã CV' . sprintf('%03d', $khcv_ma) . '', $nvthuchien, $khcv_ma);
                     $sqlhoanthanhcv = mysqli_query($conn, "UPDATE `kehoachcongviec` SET `khcv_hoanthanhcv`='".date('Y-m-d')."' WHERE `khcv_ma`='$khcv_ma'");

                } 
                else {
                    add_new_history('Thay đổi trạng thái công việc', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã thay đổi trạng thái công việc có mã CV' . sprintf('%03d', $khcv_ma) . ' từ trạng thái <b>"'. $row_nvth['khcv_tiendo'] .'"</b> sang trạng thái <b>' . $trangthaicv. '</b>', $nvthuchien, $khcv_ma);
            ...
            if ($row_nvth['khcv_noidung'] != $ndcv) {
                add_new_history('Thay đổi nội dung công việc', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã thay đổi nội dung công việc CV' . sprintf('%03d', $khcv_ma) . ' có nội dung "<b>' . $row_nvth['khcv_noidung'] . '</b>" thành nội dung mới "<b>' . $ndcv . '</b>"', $nvthuchien, $khcv_ma);
            }
            $result = mysqli_query($conn, $sql);
            if ($result) {
                ...
                $data[] = [
                    'alert'  =>   'error:Cập nhật công việc thất bại!',
            ...
            nvlkh.nv_ma as nvlkh_ma, nvlkh.nv_hoten as nvlkh_hoten, nvlkh.nv_email as nvlkh_email, 
                duan.da_ma as maduan, duan.da_ten as tenduan, duan.da_thoihan, duan.da_trangthai, 
                khcv.khcv_ma as ma_cv, khcv.khcv_thuoc_ma as khcv_thuoc_macha, khcv.khcv_noidung as noidung_cv, khcv.khcv_ghichu as ghichu_cv, khcv.khcv_ngaybatdau as ngaybatdau_cv, khcv.khcv_thoihanhoanthanh as thoihanhoanthanh_cv, khcv.khcv_tiendo as tiendo_cv, khcv.da_ma as duan_cv, khcv.nv_thuchien as nvth_cv, khcv.nv_lapkehoach as nvlkh_cv
                FROM `kehoachcongviec` khcv INNER JOIN `nhanvien` nvth ON nvth.nv_ma=khcv.nv_thuchien INNER JOIN nhanvien nvlkh ON nvlkh.nv_ma=khcv.nv_lapkehoach INNER JOIN `duan` ON khcv.da_ma=duan.da_ma  WHERE nvth.nv_ma='$manvht'
    EOT;
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                $nhatky = shownhatky($row['ma_cv']);
    ...
    case 3:
        ...
        add_new_history('Hủy bỏ công việc', 'NV' . sprintf('%03d', $nvthuchien) . ' Đã hủy công việc CV' . sprintf('%03d', $khcv_ma) . ' do ' . $row_nvth['nv_thuchien'] . ' thực hiện', $nvthuchien, $khcv_ma);
...
EOT;
        ...
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            $tontuan = timtontuan($row['ma_cv']);
            // xulykhcvchacon($row['ma_cv']);
            $trehan= trehan_khcvcon($row['ma_cv']);
            if(strcasecmp($row['tiendo_cv'],'Đã hủy')==0){
                $data[] = [
                    'alert' =>  '',
                    'khcv_stt' =>  $i++,
                    'khcv_ma' =>  '<div class="khcv_ma disabledbutton" data-id="' . $row['ma_cv'] . '" >CV' . sprintf("%03d", $row['ma_cv']) . '</div>',
                    'khcv_noidung' =>  '<div class="khcv_noidung disabledbutton" data-id="' . $row['noidung_cv'] . '" >' . $row['noidung_cv'] . '</div>',
                    'khcv_ghichu' =>  '<div class="khcv_ghichu disabledbutton" data-id="' . $row['ghichu_cv'] . '" >' . $row['ghichu_cv'] . '</div>',
                    'khcv_ngaybatdau' =>  '<div class="khcv_ngaybatdau disabledbutton" data-date="' . $row['ngaybatdau_cv'] . '" >' . date('d-m-Y', strtotime($row['ngaybatdau_cv'])) . '</div>',
                    'khcv_thoihanhoanthanh' =>  '<div class="khcv_thoihanhoanthanh disabledbutton" data-date="' . $row['thoihanhoanthanh_cv'] . '" >' . date('d-m-Y', strtotime($row['thoihanhoanthanh_cv'])) . '</div>',
                    'khcv_tiendo' =>  '<div class="khcv_tiendo disabledbutton" data-id="' . $row['tiendo_cv'] . '" >' . $row['tiendo_cv'] . '</div>',
                    'da_ten' =>  '<div class="da_ten disabledbutton" data-id="' . $row['maduan'] . '" title="' . $row['tenduan'] . '" >PB' . sprintf("%03d", $row['maduan']) . '</div>',
                    'khcv_nvthuchien' =>  '<div class="khcv_nvthuchien disabledbutton" data-nvlkh="' . $row['nvlkh_ma'] . '" data-id="' . $row['nvth_ma'] . '" >' . $row['nvth_hoten'] . '</div>',
                    'khcv_nvlkh' =>  '<div class="khcv_nvlkh disabledbutton"  data-id="' . $row['nvlkh_ma'] . '" data-nvthuchien="' . $row['nvth_ma'] . '" >' . $row['nvlkh_hoten'] . '</div>',
                    'khcv_thuoc_macha' =>  !empty($row['khcv_thuoc_macha']) ? '<div class="khcv_thuoc_macha  disabledbutton"   data-id="' . $row['khcv_thuoc_macha'] . '">CV' . sprintf('%03d', $row['khcv_thuoc_macha']) . '</div>' : '<div class="khcv_thuoc_macha disabledbutton"></div>',
                ...
                $data[] = [
                    ...
                    'da_ten' =>  '<div class="da_ten" data-id="' . $row['maduan'] . '" title="' . $row['tenduan'] . '" >PB' . sprintf("%03d", $row['maduan']) . '</div>',
                    'khcv_nvthuchien' =>  '<div class="khcv_nvthuchien" data-nvlkh="' . $row['nvlkh_ma'] . '" data-id="' . $row['nvth_ma'] . '" >' . $row['nvth_hoten'] . '</div>',
                    'khcv_nvlkh' =>  '<div class="khcv_nvlkh"  data-id="' . $row['nvlkh_ma'] . '" data-nvthuchien="' . $row['nvth_ma'] . '" >' . $row['nvlkh_hoten'] . '</div>',
                    'khcv_thuoc_macha' =>  !empty($row['khcv_thuoc_macha']) ? '<div class="khcv_thuoc_macha"   data-id="' . $row['khcv_thuoc_macha'] . '">CV' . sprintf('%03d', $row['khcv_thuoc_macha']) . '</div>' : '<div class="khcv_thuoc_macha"></div>',
                    'ton_tuan' =>   $tontuan . $trehan,
                    'nhatky' =>  '<div class="nhatky" data-id="' .   $row['ma_cv'] . '"  >' . $nhatky . '</div>',
